function build_rule(lang, func, tree) {
  ...
  function elim(tree, n, tab, free, used) {
    if (Array.isArray(tree) || typeof tree === "string") {
      ...
      for (var i = 0; i < free.length; ++i) {
        code += tabs(tab) + "free(get_args("+free[i]+"));\n";
      }
      var done = make(tree, used);
      for (var arg in used) {
        if (used[arg] === false) {
          code += tabs(tab) + "collect("+arg+");\n";
        }
      }
      code += tabs(tab) + "return " + done + ";\n";
    } else {
      code += tabs(tab) + VAR+" arg"+n+" = "+NOD("get_args(func)",n)+";\n";
      if (tree._) {
        ...
        for (var ctor in tree) {
          ...
          //for (var i = 0; i < kind_to_arity[name_to_kind[ctor]]; ++i) {
            code += tabs(tab+4)+VAR+" arg"+n+"_"+i+" = "+NOD("get_args(arg"+n+")",i)+";\n";
  ...
  function make(node, used) {
    if (typeof node === "string") {
      ...
      return "make"+(node.length-1)+"("+node[0]+vals+")";
    }
  }
  elim(tree, 0, 6, ["func"], {});
  code += tabs(4) + "}";
